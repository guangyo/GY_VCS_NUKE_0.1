...
class ct_base(ct_plu.extend):
    ...
    #是否为提交检查的文件框
    def is_submit(self, a_tw, a_db, a_filebox_id):
        dic={"db":a_db, "id":a_filebox_id, "field":"is_submit"}
        res=a_tw.con()._send("c_file","get_one_with_id", dic)
        if unicode(res).strip().lower()=="y":
            return True
        return False        
    ...
    def run(self, a_dict_data):
        ...
        try:
            t_tw = tw()
            #如果不是提交检查的文件框。直接跳过
            if not self.is_submit(t_tw, t_db, t_filebox_id):
                return self.ct_true()
            ...
            if t_res==True:
                ...
                return self.ct_false("Unassigned, Can not submit")
